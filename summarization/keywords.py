...
INCLUDING_FILTER = ['N', 'V', 'A'] #nouns, verbs, adjectives
EXCLUDING_FILTER = []
...
def _get_words_for_graph(tokens, pos_filter=None):
    ...
    for word, unit in iteritems(tokens):
        if exclude_filters and unit.tag in exclude_filters:
            continue
        if not include_filters or not unit.tag or unit.tag[0] in include_filters:
...
def keywords(text, ratio=0.2, words=None, split=False, scores=False, pos_filter=('N', 'V', 'A'),
    ...
    tokens = _clean_text_by_word(text, deacc=False)
