...
def clean_text_by_sentences(text):
    ...
    if isinstance(text, list):
        ...
        raise ValueError("Input is neither list of sentences nor string")
...
def clean_text_by_word(text, deacc=False):
    ...
    tags = tagger.tag_tnt(join_words(original_words))
    #Takes too long
    #tags=None
    #if HAS_PATTERN:
...
def tokenize_by_word(text):
    ...
    """
    text_without_acronyms = replace_with_separator(text, "", [AB_ACRONYM_LETTERS])
    return tokenize(text_without_acronyms, to_lower=True, deacc=False)
